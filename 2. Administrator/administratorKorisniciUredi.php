<?php
	...
	try
	{
		...
		$q1 = mysql_query("SELECT imePrezime, korisnickoIme, lozinka, email, telefon, privilegija, odjel_idOdjel FROM korisnik WHERE idKorisnik = '$_POST[idKorisnik]';") or die("Error in query: ".mysql_error());
		
		while ($row = mysql_fetch_assoc($q))
		{
			// Dobavljene podatke ispisati na formu za Uređivanje
			// Primjer kako se koristi dobavljena vrijednost: $row['imePrezime']
...
<?php
	...
	try
	{
		...
		$q2 = mysql_query("UPDATE korisnik SET imePrezime = '$_POST[imePrezime]', korisnickoIme = '$_POST[korisnickoIme]', lozinka = '$_POST[lozinka]', email = '$_POST[email]', telefon = '$_POST[telefon]', privilegija = '$_POST[privilegija]', odjel_idOdjel = '$IDOdjela' WHERE idKorisnik = '$_POST[idKorisnik]';") or die("Error in query: ".mysql_error());				
					
		mysql_close();
		
		echo "Korisnik je uspješno izmijenjen!";
	}
	catch (Exception $e) 
	{
		echo 'Caught exception: ', $e->getMessage(), "\n";
